...
const UploadedDocuments = ({ caseDetails }) => {
  ...
  const handleChatOnSingleDocument = (dockDetails) => {
    try {
  ...
  return (
    <card extra={"w-full pb-10 p-4 h-full"}>
                ...
                >
                  ...
                  <td class="border-b p-3">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => handleChatOnSingleDocument(uploadedDocument)}
                    >
                      Chat
      ...
      {/* single chat popup here  */}
      {/* {isPopupOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-700px h-900px flex flex-col">
            <h2 className="text-xl font-semibold mb-4 flex-shrink-0">Chat on Document</h2>
            <div id="chatContent" className="flex-grow overflow-y-auto">
              <SingleDocumentChat caseDetails={caseDetails} />
            </div>
            <button onClick={closePopup} className="mt-4 p-2 bg-red-500 text-white rounded flex-shrink-0">
              Close
            </button>
          </div>
        </div>
